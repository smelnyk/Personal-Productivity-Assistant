...
def load_browsers_list():
    ...
    """
    try:
        with open(os.path.join(LOG_FOLDER_BASE, BROWSERS_FILE), 'r') as file:
...
def load_productive_list():
    ...
    """
    try:
        with open(os.path.join(LOG_FOLDER_BASE, PRODUCTIVE_FILE), 'r') as file:
            return [line.strip() for line in file]
    except FileNotFoundError:
...
def load_unproductive_list():
    ...
    """
    try:
        with open(os.path.join(LOG_FOLDER_BASE, UNPRODUCTIVE_FILE), 'r') as file:
            return [line.strip() for line in file]
    except FileNotFoundError:
